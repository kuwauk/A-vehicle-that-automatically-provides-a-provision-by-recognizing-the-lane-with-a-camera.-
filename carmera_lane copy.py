...
class RPLidarC1:
    ...
    def _read_loop(self):
        while self._running:
            ...
            dist = raw_dist / 4.0
            quality = data[4] >> 2
            if quality >= QUALITY_THRESHOLD and dist > 199:
                with self._lock:
    ...
    try:
        while True:
            ...
            if camera_mode:
                ...
                # ----------------- 라이다 거리 확인 -----------------
                dists = lidar.get_distances()  # quality ≥ 4 필터 적용
                min_dist = min(dists, default=float('inf'))

                cv2.putText(overlay, f"Min Dist: {min_dist:.1f} mm", (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 0, 0), 2)
